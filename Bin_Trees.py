...
class Tree:
    ...
    def has_duplicate(self):
        ...
        for c in self.inorder():
            for s in self.inorder():
                if c._element == s._element:
                    count += 1
            if count >= 2:
                return True
            else:
...
def is_isomorphic(tree1, tree2):
    ...
    # case1: node1 and node2 are both none, then they are isomorphic
        if node1 is None and node2 is None:
            return True
    
    # case2: node1 or node2 are either none, then they are not isomorphic
        if node1 is None or node2 is None:
            return False
    
        if node1._element != node2._element:
            return False
        # There are two possible cases for tree1 and tree2 to be isomorphic
        # case1: The subtrees rooted at these nodes have NOT
        # been "Flipped".
        # Both of these subtrees have to be isomorphic
        # case2: The subtrees rooted at these nodes have
        # been "Flipped"
        return ((helper(node1._left,node2._left) and helper(node1._right, node2._right))
            or(helper(node1._left,node2._right) and helper(node1._right,node2._left)))
    return helper(tree1._root,tree2._root)


# Task 3 
def build_expression_tree(infix):
    """
    :param infix: String -- infix string contains spaces between each operand/operator.

    :return: a class Tree object. This tree should be the Expression Tree for the given infix string.
    """
    # TODO, modify the code below, this is just place holder code.
    temp1 = infix.split()
    temp2 = [int(temp1[i]) if temp1[i].isdigit() else temp1[i] for i in range(len(temp1))]
    temp = []
    for c in temp2:
        newtree = Tree()
        newtree.add_root(c)
    ...
    for priority in range(3,0,-1):
        i = 1
        while i < len(temp):
            if p[temp[i].root()._element] == priority:
                cur = temp[i]
                cur._attach(cur.root(), temp[i-1],temp[i+1])
                del temp[i-1]
                del temp[i-1]
                temp[i-1] = cur
                continue
...
# Tasl 4
def zero_one_update(one_zero_tree):
    """
    :param one_zero_tree: a class Tree object. This tree has been initialized such that
                          leaves have 0/1 and internal nodes have None
    Update the elements in all the nodes.
    :return: None or the one_zero_tree
    """
    def helper1_postorder(one_zero_tree):
        if not one_zero_tree.is_empty():
            for node, depth in helper2_subtree_postorder(one_zero_tree._root,0):
                yield node, depth
    def helper2_subtree_postorder(node, depth):
        for c in one_zero_tree.children(node):
            for other, depth2, in helper2_subtree_postorder(c, depth+1):
                yield other, depth2
        yield node, depth
    for c,k in helper1_postorder(one_zero_tree):
        if c._element is None:
            # for t in one_zero_tree.children(c):
            #     if t is None:
            #         continue
            #     else:
            if k%2 == 0:
                c._element = max(chi._element for chi in one_zero_tree.children(c))
            else:
                c._element = min(chi._element for chi in one_zero_tree.children(c))
        else:
            continue
...
def main():
    ...
    ###################### Generate sample tree 7 #######################
    print("g")
    g = Tree()
    g_root = g.add_root(None)
    gl1 = g.add_left(g_root, None)
    gr1 = g.add_right(g_root, None)
    gr2 = g.add_right(gr1, 0)
    gl2 = g.add_left(gl1, None)
    glr = g.add_right(gl1, 1)
    gl3 = g.add_left(gl2, 0)
    gl2r= g.add_right(gl2, None)
    gl2rl = g.add_left(gl2r, 1)
    gl2r2 = g.add_right(gl2r, 0)
    # pretty_print(g)

    ###################### Generate sample tree 8 #######################
    print("gg")
    gg = Tree()
    gg_root = gg.add_root(0)
    ggl1 = gg.add_left(gg_root, 0)
    ggr1 = gg.add_right(gg_root, 0)
    ggr2 = gg.add_right(ggr1, 0)
    ggl2 = gg.add_left(ggl1, 0)
    gglr = gg.add_right(ggl1, 1)
    ggl3 = gg.add_left(ggl2, 0)
    ggl2r = gg.add_right(ggl2, 0)
    ggl2rl = gg.add_left(ggl2r, 1)
    ggl2r2 = gg.add_right(ggl2r, 0)
    # pretty_print(gg)
    ...
    print("#-------------------------- Problem 4 one zero trees... --------------------------")
    zero_one_update(g)
    pretty_print(g)
    print("updated tree g Expected:")
    pretty_print(gg)
