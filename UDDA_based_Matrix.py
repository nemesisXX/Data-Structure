...
class UDDA_Matrix:
    ...
    def max_value(self):
        ...
        if self.r==0 or self.c==0:
            return None
        curr = self._theRows[0][0]
        for i in range(self.r):
            for k in range(self.c):
                if self._theRows[i][k] > curr:
                    curr = self._theRows[i][k]
        return curr
    ...
    def scaleBy(self, scalar):
        ...
        for i in range(self.r):
            for k in range(self.c):
                self._theRows[i][k] = scalar*self._theRows[i][k]

    # Creates and returns a new matrix that is the transpose of this matrix.
    def transpose(self):
        # To do ......
        new_matrix = UDDA_Matrix(self.c,self.r)
        for i in range(self.c):
            for j in range(self.r):
                new_matrix [i,j] = self._theRows[j][i]
        return new_matrix
    ...
    def __add__(self, rhsMatrix):
        # To do ......
        new_matrix= UDDA_Matrix(self.r,self.c)
        if self.c!=rhsMatrix.c or self.r!=rhsMatrix.r:
            raise ValueError
        for i in range(self.r):
            for k in range(self.c):
                value = self._theRows[i][k]+rhsMatrix._theRows[i][k] # add each entries in the proper position
    ...
    def __mul__(self, rhsMatrix):
        # To do ......
        result = UDDA_Matrix(self.r,rhsMatrix.c)
        if self.c!=rhsMatrix.r:
            raise ValueError
        # iterate through rows of X
        for i in range(self.r):
            # iterate through columns of Y
            for j in range(rhsMatrix.c):
                # iterate through rows of Y
                for k in range(rhsMatrix.r):
                    result[i,j] += self._theRows[i][k] * rhsMatrix._theRows[k][j]
        return result
